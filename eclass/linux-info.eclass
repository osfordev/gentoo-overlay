#
# From https://github.com/gentoo/gentoo/blob/1880c37156b3a1f51279acfbc97375821f275b61/eclass/linux-info.eclass
#
...
# @FUNCTION: linux_config_srcuserdefined_exists
# @RETURN: true or false
# @DESCRIPTION:
# It returns true if a file ${KCONFIG_CONFIG} exists, otherwise false
linux_config_srcuserdefined_exists() {
	export _LINUX_CONFIG_EXISTS_DONE=1
	use kernel_linux && [[ -n "${KCONFIG_CONFIG}" && -s "${KCONFIG_CONFIG}" ]]
}
...
# functions.
linux_config_exists() {
	linux_config_srcuserdefined_exists || linux_config_src_exists || linux_config_bin_exists
}
...
# then return false.
linux_config_path() {
	if linux_config_srcuserdefined_exists; then
		echo "${KCONFIG_CONFIG}"
	elif linux_config_src_exists; then
...
require_configured_kernel() {
	...
	if ! linux_config_src_exists && ! linux_config_srcuserdefined_exists; then
		qeerror "Could not find a usable nor .config in the kernel source directory nor ${KCONFIG_CONFIG}."
